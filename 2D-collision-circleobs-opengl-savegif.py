...
obstacle_center = np.array([0.5, 0.0, 0.0])
inner_radius = 0.2
outer_radius = 0.4
...
def main():
    ...
    for frame in range(num_frames):
        ...
        # Check for particle-hollow circle collision and resolve it
        ...
        save_frame(frame_filename)
        frames.append(frame_filename)
        pygame.time.wait(int(dt * 1000))

    # Save frames as a GIF with equal duration for all frames and set loop to 0
    images = [imageio.imread(frame) for frame in frames]
    imageio.mimsave('simulation.gif', images, fps=30, loop=0)  # Set duration for each frame and loop to 0
